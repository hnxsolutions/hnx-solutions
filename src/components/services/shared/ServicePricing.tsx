...
export default function ServicePricing({ service }: ServicePricingProps) {
  return (
    <section
      id="pricing"
      className="relative px-5 py-14 sm:px-6 lg:px-8 xl:px-10 2xl:px-12"
    >
      <div className="mx-auto w-full max-w-[min(92vw,1440px)] px-5 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
        ...
        >
          {service.packages.map((pkg) => (
            ...
            >
              <div
                className={`absolute -right-20 -top-20 h-48 w-48 rounded-full bg-gradient-to-br ${service.accent} opacity-10 blur-3xl`}
              />
              ...
              <div
                className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${service.accent} ${
                  pkg.popular ? "opacity-100" : "opacity-55"
                }`}
              />

              {pkg.popular ? (
                <span className="absolute right-5 top-5 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-[#145cb7] dark:border-cyan-300/25 dark:bg-cyan-300/10 dark:text-cyan-200">
                  Most Popular
                </span>
              ) : null}

              <h3
                className="relative pr-24 text-2xl font-semibold tracking-[-0.035em] text-slate-950 antialiased dark:text-white"
                style={{ transform: "translateZ(16px)" }}
              >
                {pkg.name}
              </h3>

              <p className="relative mt-3 min-h-14 text-sm font-normal leading-6 text-slate-600 antialiased dark:text-slate-300">
                {pkg.description}
              </p>

              <p
                className={`relative mt-6 bg-gradient-to-r ${service.accent} bg-clip-text text-4xl font-semibold tracking-[-0.045em] text-transparent antialiased`}
              ...
              <div className="relative mt-6 grid gap-3">
                {pkg.features.map((feature) => (
                  <div
                    key={feature}
                    className="flex gap-3 text-sm font-normal leading-6 text-slate-700 dark:text-slate-300"
                  >
              ...
              <Link
                href="/contact"
                className={`group relative mt-7 inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl px-5 py-3.5 text-sm font-semibold transition hover:-translate-y-0.5 ${
                  pkg.popular
