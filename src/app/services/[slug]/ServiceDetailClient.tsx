...
export default function ServiceDetailClient({
  ...
  }, [packages.length, pauseAutoSlideTemporarily, updateActivePackage]);
  ...
  useEffect(() => {
    ...
    autoSlideRef.current = setInterval(() => {
      setActivePackageIndex((prev) => {
        const nextIndex = (prev + 1) % packages.length;
  ...
  }, [isAutoSlidingPaused, packages.length, scrollToPackage]);
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <div className="relative xl:hidden">
                <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-6 bg-linear-to-r from-(--bg) to-transparent" />
                <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-6 bg-linear-to-l from-(--bg) to-transparent" />

                <div
                  ref={pricingSliderRef}
                  className="flex snap-x snap-proximity gap-3 overflow-x-auto px-1 pb-3 pt-2 scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
                >
                  {packages.map((pkg, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="basis-[80%] shrink-0 snap-center"
                      >
                        <div
                          className={`group relative h-full overflow-hidden rounded-[1.45rem] border p-4 backdrop-blur-2xl transition-all duration-500 ${
                            pkg.popular
                              ? "border-primary/30 bg-[linear-gradient(180deg,rgba(56,189,248,0.08),rgba(255,255,255,0.55))] shadow-[0_16px_44px_rgba(56,189,248,0.10)] dark:bg-[linear-gradient(180deg,rgba(56,189,248,0.12),rgba(255,255,255,0.04))] dark:shadow-[0_16px_44px_rgba(56,189,248,0.14)]"
                              : "glass-card border-(--border)"
                          }`}
                        >
                          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(56,189,248,0.10),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(139,92,246,0.07),transparent_26%)] opacity-70" />

                          {pkg.popular ? (
                            <div className="absolute right-3 top-3 rounded-full border border-primary/25 bg-primary/15 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.15em] text-primary">
                              Popular
                            </div>
                          ) : null}
                          ...
                          <div className="relative z-10">
                            <div className="pr-18">
                              ...
                              <p className="mt-2 line-clamp-2 text-xs leading-5 text-(--text-soft)">
                                {pkg.description}
                              </p>
                            </div>

                            <div className="mt-4 rounded-xl border border-(--border) bg-white/35 p-3 dark:bg-dark-900/35">
                              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-(--text-soft)">
                                Package Price
                              </p>
                              <p className="mt-1.5 text-xl font-extrabold text-cyan-500 dark:text-cyan-300">
                                {pkg.price}
                              </p>
                            </div>
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2">
                              {pkg.features.map((feature) => (
                                <div
                                  key={feature}
                                  className="grid grid-cols-[16px_minmax(0,1fr)] items-start gap-1.5"
                                >
                                  <span className="mt-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full bg-emerald-400/10 text-emerald-500 dark:text-emerald-300">
                                    <HiCheck size={10} />
                                  </span>
                                  <p className="text-[11px] leading-4.5 text-(--text-muted)">
                                    {feature}
                                  </p>
                                </div>
                              ))}
                            ...
                            <div className="mt-5">
                              <Link
                                href="/contact"
                                className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-bold transition-all duration-300 hover:-translate-y-0.5 ${
                                  pkg.popular
                                    ? "bg-linear-to-r from-primary to-accent text-dark-900 shadow-[0_10px_24px_rgba(56,189,248,0.20)]"
                          ...
                          <div
                            className={`pointer-events-none absolute -bottom-10 -right-4.5 h-24 w-24 rounded-full blur-3xl transition-all duration-500 ${
                              pkg.popular
                          ...
                          />
                          <div
                            className={`pointer-events-none absolute -inset-px rounded-[1.45rem] border transition-opacity duration-300 ${
                              isActive
              ...
              >
                {packages.map((pkg) => (
                  <motion.div
                    ...
                    className={`group relative overflow-hidden rounded-[1.6rem] border p-5 backdrop-blur-2xl ${
                      pkg.popular
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="mt-5 rounded-2xl border border-(--border) bg-white/35 p-4 dark:bg-dark-900/35">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-(--text-soft)">
                          Package Price
                        </p>
